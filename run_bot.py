            ...
            for entry in feed.entries[:20]:
                ...
                if hasattr(entry, "published"):
                    ...
                    try:
                        ...
                        data_publicacao = parsedate_to_datetime(
                            entry.published
                        )
                ...
                if data_publicacao:
                    ...
                    horas_passadas = (
                        (agora - data_publicacao).total_seconds() / 3600
                    )

                    if horas_passadas > limite_horas:
                        continue

                conteudo = remover_acentos(
                    f"{titulo} {resumo}".lower()
                )
...
def executar_modo_teste(tema_forcado=None, publicar=False):
    ...
    if "stf" in texto_total or "supremo" in texto_total:
        ...
        query_visual = (
            "Supremo Tribunal Federal Brasília Brazil building"
        )

    elif "senado" in texto_total:

        query_visual = (
            "Senado Federal Brasília Brazil congress building"
        )

    elif "câmara" in texto_total or "camara" in texto_total:

        query_visual = (
            "Câmara dos Deputados Brasília Brazil congress building"
        )

    elif "planalto" in texto_total:

        query_visual = (
            "Palácio do Planalto Brasília Brazil government palace"
        )

    elif "khamenei" in texto_total or "irã" in texto_total or "ira" in texto_total:

        query_visual = (
            "Khamenei Iran supreme leader portrait Tehran"
        )
...
if __name__ == "__main__":
    ...
    if os.getenv("TEST_MODE") == "true":
        ...
        tema_teste = os.getenv(
            "TEST_TEMA",
            "policial"
        )

        publicar_teste = (
            os.getenv("TEST_PUBLICAR", "false") == "true"
        )
    ...
    print(
        f"Post publicado com sucesso: {noticia['titulo']}"
    )
